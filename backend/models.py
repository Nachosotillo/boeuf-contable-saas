...
class NominaProgramacion(Base):
    """
    Registra cuándo debe ejecutarse el próximo asiento de nómina.
    Una fila por empresa. El router de nómina la crea al añadir el primer
    empleado y la actualiza cada vez que se genera un asiento.
    """
    __tablename__ = "nomina_programacion"
    ...
    proxima_fecha: Mapped[date] = mapped_column(Date, nullable=False)
    ultima_ejecucion: Mapped[Optional[date]] = mapped_column(Date)
    intervalo_dias: Mapped[int] = mapped_column(Integer, default=30)
    creado_en: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
